...
export class ApiService {
  ...
  getCaseById(id: string): Observable<Cases> {
    const url = `${apiUrl}/${id}`;
  ...
  updateCases(id: string, cases: Cases): Observable<Cases> {
    const url = `${apiUrl}/${id}`;
    return this.http.put<Cases>(url, cases, httpOptions)
      .pipe(
        tap(_ => console.log(`updated cases id=${id}`)),
        catchError(this.handleError<Cases>(`updatedCases id=${id}`))
      );
  }

  deleteCase(id: string): Observable<Cases> {
    const url = `${apiUrl}/${id}`;
    return this.http.delete<Cases>(url, httpOptions)
      .pipe(
        tap(_ => console.log(`deleted cases id=${id}`)),
        catchError(this.handleError<Cases>(`deleteCase id=${id}`))
      )
  }

  getCasesStatus(status: string): Observable<Statistics> {
    const url = `${apiUrl}/daily/${status}`;
    return this.http.get<Statistics>(url)
      .pipe(
        tap(_ => console.log(`fetched statistics status =${status}`)),
        catchError(this.handleError<Statistics>(`getCasesStatus`))
      );
  }
